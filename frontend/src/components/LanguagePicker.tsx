...
export default function LanguagePicker({ visible, selected, onSelect, onClose }: Props) {
  const [query, setQuery] = useState("");

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return LANGUAGES;
    return LANGUAGES.filter((l) => l.label.toLowerCase().includes(q) || l.code.includes(q));
  }, [query]);
  ...
  return (
    ...
    >
      <Pressable style={styles.backdrop} onPress={onClose}>
        <Pressable style={styles.sheet} onPress={(e) => e.stopPropagation()} testID="language-picker-sheet">
          <View style={styles.header}>
            <Text style={styles.title}>Affirmation Language</Text>
            <TouchableOpacity testID="language-picker-close" onPress={onClose} hitSlop={16}>
          ...
          <View style={styles.searchBar}>
            ...
            />
            {query.length > 0 && (
              <TouchableOpacity testID="language-picker-search-clear" onPress={() => setQuery("")} hitSlop={10}>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <Text style={styles.emptyText} testID="language-picker-empty">No languages match &quot;{query}&quot;</Text>
            }
            renderItem={({ item }) => {
